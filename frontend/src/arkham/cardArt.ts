...
export function cardFrontImage(card: CardDef, ignoreVariants = false): string {
  const art = (code: string) => cardImg(code, ignoreVariants)

  if (card.cardType === 'LocationType' && card.doubleSided) return art(`${card.art}b`)
  if (isBackPrimary(card)) return art(card.otherSide!.replace(/^c/, ''))
  return art(card.art)
}

export function cardBackImage(card: CardDef, ignoreVariants = false): string {
  const { cardType, otherSide, doubleSided } = card
  const art = (code: string) => cardImg(code, ignoreVariants)

  if (isBackPrimary(card)) return art(card.art)
  if (otherSide) return art(otherSide.replace(/^c/, ''))
  ...
  if (!cardType) return art(`${card.art}b`)

  if (['ActType', 'AgendaType', 'ScenarioType', 'InvestigatorType'].includes(cardType))
    return art(`${card.art.replace(/a$/, '')}b`)

  if (cardType === 'LocationType') {
    if (doubleSided) return art(card.art)
    return imgsrc(ENCOUNTER_BACK)
  }

  if (['EnemyType', 'StoryType'].includes(cardType) && doubleSided) return art(`${card.art}b`)

  if (doubleSided) return art(`${card.art.replace(/a$/, '')}b`)
...
export function hasCardBackArt(card: CardDef, ignoreVariants = false): boolean {
  const back = cardBackImage(card, ignoreVariants)
