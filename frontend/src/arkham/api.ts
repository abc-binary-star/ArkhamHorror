...
export type StoredCustomCard = { id: string; setId: string; cardCode: string; def: any; art: string | null; updatedAt: string }
...
export const saveCustomCard = async (card: { setId: string; def: any; art: string | null }): Promise<StoredCustomCard> => {
...
export const importCustomCardSet = async (payload: {
  ...
  return { set: data.set, cards: data.cards.map((row: any) => ({ id: row.id, ...row })) }
}
