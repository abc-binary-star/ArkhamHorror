...
const { t } = useI18n({ useScope: 'local', messages: {
  ...
  en: {
    drawTitle: 'Draw an encounter card',
...
const visible = computed(() => !!kind.value && !props.suppressed && !uiLock.value && !phaseAnnouncement.value)
...
<template>
  <Teleport to="body">
    ...
    <dialog ref="dialog" class="action-dialog" :aria-labelledby="titleId" @cancel.prevent="collapse">
      ...
      <AtmosphereLine :tone="kind === 'draw' ? 'encounter' : tokens?.damage && tokens?.horror ? 'peril' : tokens?.horror ? 'horror' : 'damage'" />
