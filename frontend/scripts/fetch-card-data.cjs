#!/usr/bin/env node
/*
 * Downloads ArkhamDB card metadata exports into frontend/card-data/cards_*.json.
 *
...
if (hasArg('--help') || hasArg('-h')) {
  console.log(`Usage: node scripts/fetch-card-data.cjs [--langs en,zh,fr] [--slim]\n\nDownloads ArkhamDB /api/public/cards/?encounter=1 exports to frontend/card-data/cards_*.json.\n`)
  process.exit(0)
}

const root = path.join(__dirname, '..')
const sourceDir = path.join(root, 'card-data')
...
async function fetchJson(lang) {
  ...
  try {
    ...
    const out = path.join(sourceDir, `cards_${lang}.json`)
    const tmp = `${out}.tmp`
