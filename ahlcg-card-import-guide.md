...
- **taboo 双分**：up主偶发给 taboo 前后两个评分（如四循环「秘密卷轴：taboo前1/taboo后4」），表内打分取实战用的 taboo 后分，简介开头保留"taboo前评分：N，taboo后评分：N（表内打分取taboo后）"字样。
- **多职阶卡**：职业选项没有"多职阶"，0级多职阶卡（如四循环 .45汤姆逊/附魔剑/恐怖图腾）职业暂录「中立」；升级版带（蓝）等后缀时录对应职阶。
- up主语言习惯：投币机=换资源资产、康=取消、奶=治疗、假赛=重骰/改token、转牌库=循环抽牌。

## 3. 篇章命名与选项

- 循环系列：`N循环（循环名）（上|中|下）`，职阶对固定 上=蓝黄、中=绿紫、下=红中立（**没有"白家"这种叫法，中立就叫中立**）
- 调查员包：`调查员包（调查员名）`
...
截至 2026-09-18 共 **603 条**：
基础（上/下）80 ｜ 一循环敦威志 上中下 94 ｜ 调查员包×5 118 ｜ 二循环卡尔克萨 上中下 89 ｜ 三循环失落时代 上31 + 中29 + 下19（**上中下全部导完**） ｜ 四循环万象无终 上38 + 中30 + 下13（红，中立部分待用户继续贴） ｜ 五循环食梦者 上24 + 中28 + 下13（**上中下全部导完**）

**四循环文章结构特例**：up主把中立+多职阶卡放在（上）篇开头讲，之后按 蓝黄/绿紫/红中立 分篇；验证脚本里 select 字段返回值是**数组**（`r[pi]` 需取 `[0]`），别直接比字符串。

全表重名审计（本批新增步骤）：录入前拉全表（`--offset` 翻页，**终止条件 `len(batch)<200`**，别信 total 字段）按名字聚类，一石二鸟——既查本批撞名又排查历史重名（2026-09-18 审计结论：378 条时全表零重名）。系列规律：循环篇章=相邻两职阶×3篇（蓝黄/绿紫/红中立）。三循环选项色 Yellow。
